...
项目执行效果演示视频已录制完成，包含以下核心功能展示：

**视频文件名**：`Case RAG AI.mp4`

**📌 重点观看提示**：

1. **语义分析路由器演示**（0:00-0:30）
   - 输入"你好" → 触发 Chat 模式，直接回答
   - 输入"介绍下雇主责任险" → 触发 Direct 模式，调用检索工具获取文档信息
   - 输入"帮我画一个宇宙飞船" → 触发 Direct 模式，调用图像生成工具

2. **检索增强生成（RAG）演示**（0:30-1:00）
   - 展示检索工具从知识库中查询相关文档
   - 显示检索结果格式化输出
   - LLM 基于检索结果生成自然语言回答

3. **ReAct 多步推理演示**（1:00-1:30）
   - 输入模糊指令如"帮我看看那个东西"
   - 展示三轮工具调用尝试过程
   - 演示兜底机制触发

4. **工具扩展功能演示**（1:30-2:00）
   - 代码解释器执行数学计算
   - MCP 工具（tavily）联网搜索
   - 图像生成工具生成图片

**📺 观看方式**：
- 直接在项目根目录下找到 `Case RAG AI.mp4` 文件双击播放
- 推荐使用支持高清视频的播放器（如 VLC、PotPlayer）

**💡 注意事项**：
- 视频中使用的 API Key 已脱敏处理，实际使用时请配置有效的 API Key
- 演示环境为本地开发环境，实际部署性能可能因硬件配置不同而有所差异
