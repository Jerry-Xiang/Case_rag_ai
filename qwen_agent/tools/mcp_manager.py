...
class MCPManager:
    ...
    def initConfig(self, config: Dict):
        ...
        try:
            ...
            logger.info(f'Successfully initialized {len(result)} MCP tools')
            return result
        except Exception as e:
            logger.warning(f'Failed in initializing MCP tools: {e}. Continuing without MCP tools.')
            return []
    ...
    async def init_config_async(self, config: Dict):
        ...
        for server_name in mcp_servers:
            ...
            try:
                ...
                logger.warning(f"Failed to connect to MCP server '{server_name}': {e}. Skipping this server.")
                continue
