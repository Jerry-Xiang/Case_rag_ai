...
class Memory(Agent):
    ...
    def __init__(self,
        ...
        self.rag_backend = self.cfg.get('rag_backend', 'default')

        function_list = function_list or []
        
        print("\n" + "="*60)
        print("[Memory] 初始化检索后端...")
        print("="*60)
        print(f"[Memory] rag_backend 配置: {self.rag_backend}")
        
        if self.rag_backend == 'elasticsearch':
            try:
                print("[Memory] → 选择 Elasticsearch 后端")
                retrieval_tool = ESRetrievalTool(cfg=self.cfg)
                function_list.append(retrieval_tool)
                print("[Memory] ✓ ESRetrievalTool 已添加到工具列表")
            except Exception as e:
                print(f"[Memory] ✗ ESRetrievalTool 加载失败: {e}")
                print("[Memory] → 降级到文档解析检索 (retrieval)")
        ...
        print("[Memory] → 添加文档解析工具 (doc_parser)")
        print("="*60 + "\n")
